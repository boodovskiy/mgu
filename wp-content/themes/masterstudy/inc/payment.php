	...
	function checkPayment($data){
		...
		if (strcmp ($res, "VERIFIED") == 0) {
			...
			if( get_post_type( $invoice ) == 'event_participant' ){
				...
				$events_admin_email_subject = str_replace( array( '[event]' ), array( get_the_title( $item_number ) ), get_theme_mod( 'events_admin_email_subject', __( 'New participant for [event]', 'stm_domain' ) ));
				...
				Email: [email]
				Phone: [phone]
				Message: [message]', 'stm_domain' ) ));
				$events_participant_email_subject = str_replace( array( '[event]' ), array( get_the_title( $item_number ) ), get_theme_mod( 'events_participant_email_subject', __( 'Confirmation of your pariticipation in the [event]', 'stm_domain' ) ));
