...
function stm_startup_styles() {
	wp_enqueue_style( 'stm-startup_css', get_template_directory_uri() . '/assets/admin/css/style.css', null, STM_THEME_VERSION, 'all' );
}
...
function stm_register_startup_screen() {
	...
	$stm_admin_submenu_page_creation_method(
		$theme_name_sanitized,
		esc_html__('Demo import', 'stm_domain'),
		esc_html__('Demo import', 'stm_domain'),
		'manage_options',
		'stm_option_options&tab=20',
	...
	$stm_admin_submenu_page_creation_method(
		$theme_name_sanitized,
		esc_html__('System status', 'stm_domain'),
		esc_html__('System status', 'stm_domain'),
		'manage_options',
...
function stm_get_admin_tabs($screen='welcome') {
	...
	<div class="clearfix">
		...
		<h1><?php printf(esc_html__('Welcome to %s', 'stm_domain'), $theme_name); ?></h1>
	</div>
	<div class="stm-about-text about-text">
		<?php printf( esc_html__( '%s is now installed and ready to use! Get ready to build something beautiful. Please register your purchase to get automatic theme updates, import %1$s demos and install premium plugins. Read below for additional information. We hope you enjoy it! %s', 'stm_domain' ), $theme_name, '<a href="//youtube.com/embed/QjQdVuxXieI?rel=0?KeepThis=true&TB_iframe=true&autoplay=true" target="_blank" class="thickbox">' . esc_attr__( 'Watch Our Quick Guided Tour!', 'stm_domain' ) . '</a>' ); ?></div>
	<h2 class="nav-tab-wrapper">
		<a href="<?php echo ( 'welcome' === $screen ) ? '#' : esc_url_raw( admin_url( 'admin.php?page=' . $theme_name_sanitized ) ); ?>" class="<?php echo ( 'welcome' === $screen ) ? 'nav-tab-active' : ''; ?> nav-tab"><?php esc_attr_e( 'Product Registration', 'stm_domain' ); ?></a>
		<a href="<?php echo ( 'support' === $screen ) ? '#' : esc_url_raw( admin_url( 'admin.php?page=' . $theme_name_sanitized . '-support' ) ); ?>" class="<?php echo ( 'support' === $screen ) ? 'nav-tab-active' : ''; ?> nav-tab"><?php esc_attr_e( 'Support', 'stm_domain' ); ?></a>
		<a href="<?php echo ( 'plugins' === $screen ) ? '#' : esc_url_raw( admin_url( 'admin.php?page=' . $theme_name_sanitized . '-plugins' ) ); ?>" class="<?php echo ( 'plugins' === $screen ) ? 'nav-tab-active' : ''; ?> nav-tab"><?php esc_attr_e( 'Plugins', 'stm_domain' ); ?></a>
		<a href="<?php echo ( 'demos' === $screen ) ? '#' : esc_url_raw( admin_url( 'admin.php?page=stm_option_options&tab=20' ) ); ?>" class="<?php echo ( 'demos' === $screen ) ? 'nav-tab-active' : ''; ?> nav-tab"><?php esc_attr_e( 'Install Demos', 'stm_domain' ); ?></a>
		<a href="<?php echo ( 'system-status' === $screen ) ? '#' : esc_url_raw( admin_url( 'admin.php?page=' . $theme_name_sanitized . '-system-status' ) ); ?>" class="<?php echo ( 'system-status' === $screen ) ? 'nav-tab-active' : ''; ?> nav-tab"><?php esc_attr_e( 'System Status', 'stm_domain' ); ?></a>
	</h2>
	<?php
}